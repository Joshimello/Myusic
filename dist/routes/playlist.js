...
router.post('/', async (ctx, next) => {
    ...
    await spotifyApi.clientCredentialsGrant().then(res => { spotifyApi.setAccessToken(res.body['access_token']); console.log(res.body['access_token']); }, err => { console.log('Error', err); });
    let data = await Promise.all((await getPlaylist(rawdata.id)).map(async (song) => {
        return {
    ...
    data.forEach(song => {
        list.push(new Promise(resolve => {
            if (song.id == 'exists') {
                resolve(`${song.artist} - ${song.name}.mp3`);
            }
            else {
